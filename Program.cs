    ...
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            else if (arg[0] == "video")
            {
                ...
                if (arg[1].StartsWith("AV"))
                {
                    var data = HttpUtils.GetVideoA(arg[1]);
                    VideoPicGen.Gen(data);
                    Log("已生成");
                }
                else if (arg[1].StartsWith("BV"))
                {
                    var data = HttpUtils.GetVideoB(arg[1]);
                    VideoPicGen.Gen(data);
                    Log("已生成");
                }
                else
            ...
            else if (arg[0] == "dynamic")
            {
                ...
                var data = HttpUtils.GetDynamic(arg[1]);
            ...
            else if (arg[0] == "duser")
            {
                ...
                var data = HttpUtils.GetDynamicUid(arg[1]);
                DynamicPicGen.Gen(data);
                Log("已生成");
            }
            else if (arg[0] == "live")
            {
                ...
                var data = HttpUtils.GetLive(arg[1]);
                LivePicGen.Gen(data);
                Log("已生成");
            }
            else if (arg[0] == "nuser")
            {
                ...
                var data1 = HttpUtils.SearchUser(arg[1]);
                if (data1 == null)
                {
                    Error($"搜索不到用户：{arg[1]}");
                    return;
        ...
        switch (type)
        {
            case 49:
                ...
                if (temp[0] == ConfigUtils.Config.Command.Head)
                {
                    if (temp.Length == 1)
                    {
                        robot.SendGroupMessage(0, pack.id, new() { "错误的参数" });
                        break;
                    }
                    if (temp[1] == ConfigUtils.Config.Command.Help)
                    {
                        robot.SendGroupMessage(0, pack.id, new()
                        {
                            "BotBiliBili帮助\n" +
                            ...
                            $"{ConfigUtils.Config.Command.Head} {ConfigUtils.Config.Command.UnSubscribeLive} [UID] 取消订阅UP主的直播"
                        });
                        break;
                    }
                    else if (temp[1] == ConfigUtils.Config.Command.Video)
                    {
                        ...
                        {
                            robot.SendGroupMessage(0, pack.id, new() { "错误的视频号" });
                        }
                    }
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.Dynamic)
                    {
                        ...
                        {
                            robot.SendGroupMessage(0, pack.id, new() { "错误的动态号" });
                            break;
                        }
                        Log($"正在生成动态:{comm}的图片");
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.DynamicUser)
                    {
                        ...
                        if (!Tools.IsNumeric(comm))
                        {
                            robot.SendGroupMessage(0, pack.id, new() { "错误的UP主号" });
                            break;
                        }
                        Log($"正在生成动态:{comm}的图片");
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.Live)
                    {
                        ...
                        if (!Tools.IsNumeric(comm))
                        {
                            robot.SendGroupMessage(0, pack.id, new() { "错误的直播号" });
                            break;
                        }
                        Log($"正在生成直播:{comm}的图片");
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.LiveUid)
                    {
                        ...
                        if (!Tools.IsNumeric(comm))
                        {
                            robot.SendGroupMessage(0, pack.id, new() { "错误的UID" });
                            break;
                        }
                        Log($"正在生成直播:{comm}的图片");
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.SubscribeUid)
                    {
                        ...
                        if (ConfigUtils.Subscribes.Uids.ContainsKey(comm))
                        {
                            ...
                            {
                                robot.SendGroupMessage(0, pack.id, new() { "已经订阅过了" });
                                break;
                            }
                            list.Add(pack.id);
                        }
                        ...
                        robot.SendGroupMessage(0, pack.id, new() { "订阅成功" });
                    ...
                    else if (temp[1] == ConfigUtils.Config.Command.UnSubscribeUid)
                    {
                        ...
                        if (ConfigUtils.Subscribes.Uids.ContainsKey(comm))
                        {
                            ...
                            if (list.Contains(pack.id))
                            {
                                ...
                                robot.SendGroupMessage(0, pack.id, new() { "取消订阅成功" });
                                ConfigUtils.SaveSubscribe();
                                break;
                            }
                            else
                            {
                                robot.SendGroupMessage(0, pack.id, new() { "没有订阅过" });
                                break;
                            }
                        }
                        else
